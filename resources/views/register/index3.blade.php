	...
	<img src="/img/logo2.png" width="20%" height="10%">
      ...
      <nav id="navbar" class="navbar order-last order-lg-0">
        <ul>
          ...
          <li><a class="nav-link scrollto {{ Request:: is('registerA') ? 'active' : '' }} " href="/registerA">Package A</a></li>
          <li><a class="nav-link scrollto {{ Request:: is('registerB') ? 'active' : '' }} " href="/registerB">Package B</a></li>
		      <li><a class="nav-link scrollto {{ Request:: is('registerC') ? 'active' : '' }} " href="/registerC">Package C</a></li>
...
<section id="contact" class="contact">
  ...
  <h2>Package C </h2>
      ...
      <div>
        ...
        <form id="formpackC" action="{{ url('/registerC') }}" method="POST" onsubmit="return validateForm3()">  
          @csrf
          <div class="row">
    ...
    <div class="row">
      ...
      <select id="subjects" name="subjects" class="form-control">
        @foreach ($subjects as $subject)
        @if ($subject->title =="Bahasa Inggeris")
        <option value="{{ $subject->id }}">{{ $subject->title }}</option>
        @endif
        @endforeach
    </select>
      ...
      <select id="subjects2" name="subjects2" class="form-control">
          ...
          @endforeach
            ...
            <div class="col form-group">
                <label></label>
                <select id="subjects4" name="subjects4" class="form-control">
                    @foreach ($subjects as $subject4)
                    @if ($subject4->title =="Sains")
                    <option value="{{ $subject4->id }}">{{ $subject4->title }}</option>
                    @endif
                    @endforeach
                </select>
                      </div>
                      ...
                      <div class="col form-group">
                        <label></label>
                        <select id="subjects5" name="subjects5" class="form-control">
                            @foreach ($subjects as $subject5)
                            @if ($subject5->title =="Add Math")
                            <option value="{{ $subject5->id }}">{{ $subject5->title }}</option>
                            @endif
                            @endforeach
                        </select>
                              </div>
                              ...
                              <div class="col form-group">
                                <label></label>
                                <select id="subjects6" name="subjects6" class="form-control">
                                    @foreach ($subjects as $subject6)
                                    @if ($subject6->title =="Fizik")
                                    <option value="{{ $subject6->id }}">{{ $subject6->title }}</option>
                                    @endif
                                    @endforeach
                                </select>
                                      </div>
                                      ...
                                      <div class="col form-group">
                                        <label></label>
                                        <select id="subjects7" name="subjects7" class="form-control">
                                            @foreach ($subjects as $subject7)
                                            @if ($subject7->title =="Kimia")
                                            <option value="{{ $subject7->id }}">{{ $subject7->title }}</option>
                                            @endif
                                            @endforeach
                                        </select>
                                              </div>
          ...
          <div class="text-center"><button type="submit" class="get-started-btn scrollto back">Submit</button></div>
        </form>
      </div>
  ...
  <script src="/Jss/validationform.js"></script>
